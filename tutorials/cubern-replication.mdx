...
Whenever an objects property is changed, nlua will tell the server core (only on server-side) to tell other clients to-do the same. Tho, what if one of the clients/players have not loaded the part or it just doesnt simply exist there? In that term netisus client core will simply neglect the demand
