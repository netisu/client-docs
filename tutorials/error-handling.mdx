...
### With `Connect`
We are actively working to make error handling very easy. In functions like `Connect` or `ConnectEvent` nlua will return nil or a string. If a error occurs: then it will return a string which will be the error message. 
...
error = Cube:Connect("OnBodyEntered", __SCRIPT__, "AnotherFunction") -- the value of error will be a string as well. nlua does not support binding a single signal to multiple functions. However this is going to be allowed in near-future. It will return the same error: : 'OnBodyEntered(Signal) is already connected to a function!'
