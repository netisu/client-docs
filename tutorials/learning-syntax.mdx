...
description: 'Learn the syntax of nlua'
icon: 'seal-question'
---
...
There are 2 ways to define variables in nlua. First we can make use of the keyword `local` which tells the [interpreter](https://en.wikipedia.org/wiki/Interpreter_(computing)) to create a variable which is then followed by the name of the variable with:
```lua
local MyVariable = 2
```
...
MyVariable = "Yay!"
```
Now what nlua will do here is that it will check if `MyVariable` exists then it will try to update its value to "Yay!" and if it doesnt exist then it will create a newer variable called `MyVariable` which will hold the value `Yay!`.

### Learning about comments

Comments in nlua are lines of text in your code that the interpreter ignores during execution. They are useful for adding explanations, notes, or temporarily disabling parts of your code. nlua supports both single-line and multi-line comments.
...
```lua
-- This is a single-line comment in nlua
local MyVariable = 10  -- You can also add comments at the end of a line
```

In the example above, the comment `-- This is a single-line comment in nlua` is ignored by the interpreter. The line `local MyVariable = 10` is executed normally, and the comment following it is ignored.

#### Multi-Line Comments

For comments that span multiple lines, nlua allows the use of `--[[` to open the comment and `]]` to close it. Everything between these markers is treated as a comment:
...
Comments are a powerful tool for making your nlua code more understandable and maintainable. By using comments, you can explain your logic, leave notes for yourself or others, and manage your code more effectively.
### If statements
In nlua, the `if` statement is used to execute a block of code based on a condition. The basic syntax of an `if` statement is:
...
nlua also allows you to nest `if` statements for more complex conditions:
...
This section introduces the concept of `if` statements, explains the syntax, and provides examples to illustrate how they work in nlua.

### Loops 
Loops in nlua allow you to execute a block of code multiple times. They are useful for tasks that require repetition, such as iterating over a collection or performing an action a set number of times. nlua supports `for`, `while`, and `repeat-until` loops.
...
Loops are essential for performing repetitive tasks in nlua. By using `for`, `while`, and `repeat-until` loops, you can create efficient, readable code that handles a variety of repetitive tasks.
### Functions
...
#### Accessing objects with nlua
It is very easy to acces objects in nlua. Like:
```lua
local Cube = Map.Cube
```
