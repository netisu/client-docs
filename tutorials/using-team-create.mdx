...
description: 'Learn how to use team create in netisu'
icon: 'people-group'
---
<Info>Since team-create falls into beta functionality, only users who are subscribed to Exclusive membership can host and invite.</Info>

In this tutorial we will learn how to use team-create with netisu's workshop. Before we start: team-create has some limitations. Team-create can support maximum 5 user editing 1 game at a time. If one user has selected an object: others cannot interact with the object till the user who selected it deselects or leaves the workshop. Moreover, if any user is editing any script: the script cannot be viewed by other users and cannot be deleted till other user doesnt exit the script. 
