...
#### `admin: bool`
Admin property holds a bool value which tells if the player is a netisu admin or not

#### `developer: bool`
Developer property holds a bool value which tells if a player is the creator of the current game

#### `PremiumUser: bool`
PremiumUser property holds a bool value which tells if a player is subscribed to netisu's membership
...
```lua
local netisu = Game:GetService("Player"):GetPlayer("netisu")
netisu.Avatar["Hat1"].Rotation = Vector3.new(10, 10, 10)
```
...
local netisu_player = Players:GetPlayer("netisu")
netisu_player:Kill()
```
#### `Kick(string message)`
...
local Players = Game:GetService("Players")
local netisu_player = Players:GetPlayer("netisu")
netisu_player:Kick("You broke rule #1")
```
...
if netisu_player:OwnsItem(25) then
    printl("netisu owns it!")
end
```
...
local Players = Game:GetService("Players")
local netisu_player = Players:GetPlayer("netisu")
netisu_player:Connect("OnDeath", __SCRIPT__, "FunctionToCall")
...
local Players = Game:GetService("Players")
local netisu_player = Players:GetPlayer("netisu")
netisu_player:Connect("OnRespawned", __SCRIPT__, "FunctionToCall")
